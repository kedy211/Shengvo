...
│   ├── RecordingOverlay.swift       # 录音浮窗 (液态玻璃 + 彩色微光)
│   ├── HotKeyRecorderView.swift     # 快捷键录制 (模式选择 + 单键选择)
...
```
按下快捷键 → 开始录音 → 松开快捷键（或再次按下）
    → ASR 识别（主引擎 → 故障转移链 → Apple Speech 兜底）
